...
export default function AdminFragrancesPage() {
  ...
  async function handleToggleStock(e, f) {
    e.stopPropagation()
    e.preventDefault()
    try {
  ...
  async function handleDelete(e, id, name) {
    ...
    if (!confirm(`Delete "${name}"? This cannot be undone.`)) return
    try {
      const data = await apiFetch(`/api/fragrances/${id}`, {
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#1A1714] border border-white/10 w-full max-w-xl max-h-[90vh] overflow-y-auto p-6">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
              ...
              <input placeholder="Main Image URL" value={form.imageUrl}
                onChange={e => setForm({ ...form, imageUrl: e.target.value })}
                className="col-span-2 bg-white/5 border border-white/15 px-4 py-3 text-sm focus:outline-none focus:border-[#B8924A]" />

              {/* Sub images */}
              <div className="col-span-2">
                <p className="text-xs text-[#F5EFE6]/40 mb-2">Additional Images <span className="text-[#F5EFE6]/20">(one URL per line)</span></p>
                <textarea
                  rows={3}
                  placeholder="https://... (one per line)"
                  value={(form.subImages || []).join('\n')}
                  onChange={e => setForm({ ...form, subImages: e.target.value.split('\n').filter(u => u.trim()) })}
                  className="w-full bg-white/5 border border-white/15 px-4 py-3 text-sm focus:outline-none focus:border-[#B8924A] resize-none"
                />
              </div>
              ...
              <div className="col-span-2 flex items-center justify-between border border-white/10 px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-[#F5EFE6]/30 mt-0.5">
                    {form.inStock ? 'In Stock — customers can purchase' : 'Out of Stock — visible but cannot be purchased'}
                  </p>
                </div>
                <button type="button" onClick={() => setForm({ ...form, inStock: !form.inStock })}
              ...
              <div key={f._id} className="border border-white/10 p-4 flex flex-col gap-3 hover:border-white/20 transition-colors">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-white/10">
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={(e) => handleToggleStock(e, f)}
                      className={`text-xs uppercase tracking-[0.1em] transition-colors ${f.inStock ? 'text-emerald-400/60 hover:text-red-400' : 'text-red-400/60 hover:text-emerald-400'}`}>
                      {f.inStock ? 'Mark OOS' : 'Mark In Stock'}
                    </button>
