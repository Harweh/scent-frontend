...
export default function AdminFragrancesPage() {
    ...
    async function handleImageUpload(e, isSubImage = false) {
        const files = isSubImage ? Array.from(e.target.files || []) : [e.target.files?.[0]]
        ...
        if (isSubImage) {
            setForm(prev => ({ ...prev, subImages: [...(prev.subImages || []), ...uploaded] }))
        } else {
            setForm(prev => ({ ...prev, imageUrl: uploaded[0] }))
        }
        } catch (err) {
        setError(err.message)
        } finally {
    ...
    // Toggle inStock without deleting — just PATCH the single field
    async function handleToggleStock(f) {
        try {
        ...
        {showForm && (
            ...
            <div className="bg-[#1A1714] border border-white/10 w-full max-w-xl max-h-[90vh] overflow-y-auto p-6">
                ...
                <div className="col-span-2 flex flex-col gap-2">
                    <p className="text-xs text-[#F5EFE6]/50">Main Image</p>
                    <div className="flex items-center gap-4">
                    {form.imageUrl
                        ? <img src={form.imageUrl} alt="Main" className="w-16 h-16 object-cover border border-white/15" />
                        : <div className="w-16 h-16 flex items-center justify-center text-2xl bg-white/5 border border-white/15">{form.emoji || '🌸'}</div>
                    }
                    <label className="flex-1 cursor-pointer">
                ...
                {/* Sub images */}
                <div className="col-span-2 flex flex-col gap-2">
                    <p className="text-xs text-[#F5EFE6]/50">Additional Images <span className="text-[#F5EFE6]/25">(shown in product gallery)</span></p>
                    <div className="flex flex-wrap gap-2">
                    {(form.subImages || []).map((url, i) => (
                        <div key={i} className="relative w-16 h-16">
                        <img src={url} alt={`Sub ${i}`} className="w-full h-full object-cover border border-white/15" />
                    ...
                    <label className="w-16 h-16 flex items-center justify-center border border-dashed border-white/20 text-[#F5EFE6]/30 hover:border-[#B8924A] hover:text-[#B8924A] cursor-pointer transition-colors text-xl">
                        <input type="file" accept="image/*" multiple onChange={e => handleImageUpload(e, true)} disabled={uploading} className="hidden" />
                        +
                    </label>
                    </div>
                </div>

                {/* Stock toggle — fixed: green=in stock (right), red=out of stock (left) */}
                <div className="col-span-2 flex items-center justify-between border border-white/10 px-4 py-3">
                    <div>
                    <p className="text-sm text-[#F5EFE6]/70">Stock Status</p>
                    <p className="text-xs text-[#F5EFE6]/30 mt-0.5">
                        {form.inStock ? 'In Stock — customers can add to cart' : 'Out of Stock — visible but cannot be purchased'}
                    </p>
                    </div>
                    <button type="button" onClick={() => setForm({ ...form, inStock: !form.inStock })}
                    ...
                    {/* In stock = knob on RIGHT, Out of stock = knob on LEFT */}
        ...
        <div className="max-w-6xl mx-auto px-6 sm:px-8 py-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div key={f._id} className="border border-white/10 p-4 flex flex-col gap-3 hover:border-white/20 transition-colors">
                    ...
                    {/* Toggle on card — ONLY toggles stock, never deletes */}
                    <button onClick={() => handleToggleStock(f)} title={f.inStock ? 'Mark Out of Stock' : 'Mark In Stock'}
                        className={`relative w-10 h-5 rounded-full transition-colors duration-200 shrink-0 mt-0.5 ${f.inStock ? 'bg-emerald-500' : 'bg-red-500/70'}`}>
                    ...
                    <div className="flex gap-1">
                        ...
                        <img key={i} src={url} alt="" className="w-8 h-8 object-cover border border-white/10" />
                        ))}
                        {f.subImages.length > 4 && <span className="text-xs text-[#F5EFE6]/30 self-center ml-1">+{f.subImages.length - 4}</span>}
                    </div>
                    )}
                    <div className="flex justify-between items-center pt-2 border-t border-white/10">
                    <p className="text-sm text-[#B8924A]">₦{f.pricePerMl?.toLocaleString()}/ml</p>
