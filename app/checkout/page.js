...
export default function CheckoutPage() {
    ...
    const [form, setForm] = useState({
        ...
        address: '', state: 'Lagos', paymentMethod: 'cod',
    })
    const [submitting,   setSubmitting]   = useState(false)
    const [error,        setError]        = useState('')
    const [deliveryCalc, setDeliveryCalc] = useState({
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (data.success) {
            setDeliveryCalc({ fee: data.fee, distanceKm: data.distanceKm ?? null, method: data.method, loading: false, error: '' })
            } else {
    ...
    async function handleSubmit(e) {
        ...
        const outOfStock = items.filter(i => i.inStock === false)
        if (outOfStock.length > 0) {
        setError(`Remove out of stock items: ${outOfStock.map(i => i.name).join(', ')}`)
        return
        }
        setSubmitting(true)
        setError('')
        ...
        // Build notes from the blend recipe stored on the cart item
        // Each note gets role, percentage, and mlUsed from the AI route output
        const notes = blendItem
        ? (blendItem.blendRecipe?.recipe || []).map(r => ({
            ...
            emoji:       r.emoji || '🌿',
            role:        r.role,
            pricePerMl:  blendItem.pricing?.fragranceCost
                ? Math.round((blendItem.pricing.fragranceCost * (r.percentageNum || 33) / 100) / (r.volumeMl || 2))
                : 1,
            mlUsed:      r.volumeMl || 2,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12">
                ...
                <div>
                    ...
                    <select value={form.state} onChange={e => update('state', e.target.value)}
                        ...
                        <optgroup label="── Nigeria" className="bg-[#1C1813]">
                        {NIGERIAN_STATES.map(s => <option key={s} value={s} className="bg-[#1C1813]">{s}</option>)}
                        </optgroup>
                        <optgroup label="── International" className="bg-[#1C1813]">
                    ...
                    <p className="text-xs text-[#F5EFE6]/30">
                        ...
                        {deliveryCalc.loading
                        ? <span className="text-[#F5EFE6]/40">Calculating…</span>
                        : <>₦{deliveryFee.toLocaleString()} fee{deliveryCalc.distanceKm != null && <span className="text-[#F5EFE6]/30"> ({deliveryCalc.distanceKm}km away)</span>}</>
                        }
                    </p>
                    </div>
                </div>
