...
export default function ProductDetailPage() {
    ...
    useEffect(() => {
        ...
        apiFetch(`/api/fragrances/${id}`)
        .then(data => {
            if (data.success && data.data) setFragrance(data.data)
    ...
    if (notFound || !fragrance) return (
        ...
        <div className="text-center px-6">
            <p className="text-5xl mb-6">🌿</p>
            <h1 className="font-[family-name:var(--font-display)] text-2xl mb-3">Fragrance not found</h1>
            <p className="text-sm text-[#F5EFE6]/40 mb-8">This product may have been removed.</p>
            <Link href="/catalog" className="inline-block bg-[#B8924A] text-[#100E0B] px-8 py-3.5 text-xs uppercase tracking-[0.15em] font-medium hover:bg-[#C9A45A] transition-colors">
            Back to Collection
            </Link>
        </div>
        </main>
    ...
    const { _id, name, category, description, pricePerMl, imageUrl, color, emoji, duration, inStock, subImages } = fragrance
    const totalPrice    = (pricePerMl * volume * qty).toLocaleString()
    const displayImage  = selectedImage || imageUrl

    function handleAddToCart() {
        addItem({ fragranceId: _id, name, emoji, color, imageUrl: imageUrl || null, pricePerMl, volume, qty, inStock })
        setAdded(true)
        setTimeout(() => setAdded(false), 2500)
    }
    ...
    return (
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 py-12 sm:py-20">
            ...
            <div className="flex flex-col gap-3 md:sticky md:top-28">
                ...
                {displayImage ? (
                    <Image src={displayImage} alt={name} fill priority className="object-cover object-center" />
                ) : (
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                    ...
                    <button onClick={() => setSelectedImage(null)}
                        className={`relative w-16 h-16 shrink-0 overflow-hidden border-2 transition-colors ${!selectedImage ? 'border-[#B8924A]' : 'border-white/15 hover:border-white/30'}`}>
                        <Image src={imageUrl} alt={name} fill className="object-cover" />
                    </button>
                    )}
                    {subImages.map((url, i) => (
                    <button key={i} onClick={() => setSelectedImage(url)}
                        className={`relative w-16 h-16 shrink-0 overflow-hidden border-2 transition-colors ${selectedImage === url ? 'border-[#B8924A]' : 'border-white/15 hover:border-white/30'}`}>
                        <Image src={url} alt={`${name} view ${i + 1}`} fill className="object-cover" />
                    </button>
                    ))}
                </div>
                )}
            ...
            <div className="flex flex-col gap-8 pt-2">
                ...
                <div className="flex gap-3">
                    {ML_OPTIONS.map(size => (
                    <button key={size} onClick={() => setVolume(size)}
                        className={`flex-1 py-3 text-xs uppercase tracking-[0.12em] border transition-colors
                        ${volume === size ? 'bg-[#B8924A] border-[#B8924A] text-[#100E0B]' : 'border-white/15 text-[#F5EFE6]/50 hover:border-[#B8924A] hover:text-[#B8924A]'}`}>
                        {size}ml
                    </button>
                    ))}
                </div>
                ...
                <div className="flex items-center justify-between">
                <div>
                    <p className="text-xs uppercase tracking-[0.2em] text-[#F5EFE6]/30 mb-2">Quantity</p>
                    <div className="flex items-center border border-white/15">
                    <button onClick={() => setQty(q => Math.max(1, q - 1))} className="px-4 py-2.5 text-[#F5EFE6]/50 hover:text-[#B8924A] transition-colors">−</button>
                    <span className="px-5 text-sm min-w-[2rem] text-center">{qty}</span>
                    <button onClick={() => setQty(q => q + 1)} className="px-4 py-2.5 text-[#F5EFE6]/50 hover:text-[#B8924A] transition-colors">+</button>
                    </div>
                </div>
                <div className="text-right">
                    <p className="text-xs uppercase tracking-[0.2em] text-[#F5EFE6]/30 mb-1">Total</p>
                ...
                <button onClick={handleAddToCart} disabled={!inStock}
                    className={`w-full py-4 text-xs uppercase tracking-[0.2em] font-medium transition-colors
                    ${!inStock ? 'bg-white/5 text-[#F5EFE6]/20 cursor-not-allowed'
